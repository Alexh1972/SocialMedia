...
void print_user_feed(char *name, unsigned int feed_size) {
	...
	while (feed_size && post_node) {
		...
		if (is_user_in_friends_list(user_id, post->user_id) ||
					  ...
					  unsigned int *no_posts)
{
		...
		while (repost_node) {
			...
			if (is_user_in_friends_list(user_id, repost->user_id)) {
				if (!number_posts) {
					...
					number_posts++;
					friends_reposts = realloc(friends_reposts,
											  number_posts * sizeof(post_t *));
					friends_reposts[number_posts - 1] = repost;
				}
			}
	...
	qsort(friends_reposts, number_posts, sizeof(post_t *),
		  compare_reposts_by_creation_order);
...
void print_friends_name_reposts(char *name, unsigned int post_id) {
	...
	unsigned int user_id = get_user_id(name);
	post_t **friends_reposts = get_friends_reposts(post,
								user_id, &number_reposts);
...
void print_user_posts(char *name) {
	unsigned int number_posts = 0;
	profile_post_t **posts = get_user_profile_posts(get_user_id(name),
													&number_posts);
	for (unsigned int i = 0; i < number_posts; i++) {
		if (!posts[i]->original_post_title)
			printf("Posted: \"%s\"\n", posts[i]->post->title);
		else
