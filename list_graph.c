...
void lg_add_node(list_graph_t *graph, void *data, unsigned int data_size)
{
	...
	graph->nodes++;
	if (graph->nodes > 1) {
		graph->neighbors = realloc(graph->neighbors,
								   graph->nodes * sizeof(*graph->neighbors));
...
void lg_add_edge_unoriented(list_graph_t *graph, int src, int dest)
{
	...
	ll_add_nth_node(graph->neighbors[src], graph->neighbors[src]->size, &dest);
	ll_add_nth_node(graph->neighbors[dest],
					graph->neighbors[dest]->size, &src);
}
...
void lg_remove_edge_unoriented(list_graph_t *graph, int src, int dest)
{
	...
	ll_node_t *rm_node_src = ll_remove_nth_node(graph->neighbors[src],
																pos_src);
	ll_node_t *rm_node_dest = ll_remove_nth_node(graph->neighbors[dest],
																pos_dest);
...
static void
__lq_lowest_common_ancestor_search(list_graph_t *graph, int *level,
								   int *nodes, int *count, int current_node,
								   int current_level)
{
	...
	while (node) {
		__lq_lowest_common_ancestor_search(graph, level, nodes, count,
										   *((int *)node->data),
										   current_level + 1);
		(*count)++;
		level[*count] = current_level;
...
unsigned int
lg_lowest_common_ancestor(list_graph_t *graph, int first, int second)
{
	...
	for (int i = 0; i <= count; i++) {
		if (nodes[i] == first || nodes[i] == second) {
			if (first_index == -1) {
				...
				if ((nodes[i] == second && first_found == 1) ||
					(nodes[i] == first && second_found == 1))
					second_index = i;
			}
		}
...
static void
__lg_maximal_clique(list_graph_t *graph, unsigned int **maximal_clique,
					unsigned int *clique_size, unsigned int *build_clique,
					unsigned int *build_clique_size, hashtable_t *ht)
{
	...
	for (; i < graph->nodes; i++) {
		if (!ht_get(ht, &i)) {
			...
			if (node_can_be_added) {
				...
				__lg_maximal_clique(graph, maximal_clique, clique_size,
									build_clique, build_clique_size, ht);
				(*build_clique_size)--;
...
unsigned int
*lg_maximal_clique_containing_node(list_graph_t *graph, unsigned int node,
								   unsigned int *size) {
	hashtable_t *nodes_table = ht_create(graph->nodes, hash_function_int,
										 compare_function_ints, free_info);
	...
	__lg_maximal_clique(graph, &clique_nodes, &clique_size, build_clique,
						&build_clique_size, nodes_table);
