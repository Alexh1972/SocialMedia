...
void handle_input_posts(char *input)
{
	...
	if (!strcmp(cmd, "create")) {
		...
		unsigned int fst_repost = atoi(strtok(NULL, "\n "));
		unsigned int second_repost = atoi(strtok(NULL, "\n "));
		int ancestor = find_common_repost(post, fst_repost, second_repost);
		print_common_repost(ancestor, fst_repost, second_repost);
		...
		if (liked)
			print_liked_posts(name, repost_id, post_id);
		else
			print_unliked_posts(name, repost_id, post_id);
...
static int
compare_user_profile_posts_by_creation_order(const void *a, const void *b)
{
	const profile_post_t *first = *((profile_post_t **)a);
	const profile_post_t *second = *((profile_post_t **)b);

	if (!first->original_post_title &&
		second->original_post_title) {
		return -1;
	} else if (first->original_post_title &&
				!second->original_post_title) {
		return 1;
	}
...
**get_user_profile_posts(unsigned int user_id, unsigned int *no_posts) {
	...
	while (post_node) {
		...
		if (post->user_id == user_id) {
			if (!nr_p) {
				nr_p++;
				u_pr_pos = malloc(nr_p * sizeof(profile_post_t *));
				u_pr_pos[nr_p - 1] = create_profile_post(post, NULL);
			} else {
				nr_p++;
				unsigned int dim = sizeof(profile_post_t *);
				u_pr_pos = realloc(u_pr_pos, nr_p * dim);
				u_pr_pos[nr_p - 1] = create_profile_post(post, NULL);
			}
		}
			...
			while (repost_node) {
				...
				if (repost->user_id == user_id) {
					if (!nr_p) {
						nr_p++;
						u_pr_pos = malloc(nr_p * sizeof(profile_post_t *));
						u_pr_pos[nr_p - 1] = create_profile_post(post,
																 post->title);
					} else {
						nr_p++;
						unsigned int sz = sizeof(profile_post_t *);
						u_pr_pos = realloc(u_pr_pos,
										   nr_p * sz);
						u_pr_pos[nr_p - 1] = create_profile_post(post,
																 post->title);
					}
	...
	if (u_pr_pos)
		qsort(u_pr_pos, nr_p, sizeof(profile_post_t *),
			  compare_user_profile_posts_by_creation_order);
	if (no_posts)
		*no_posts = nr_p;
...
unsigned int add_post(char *username, char *title) {
	...
	post_t *post = create_post(current_post_id, title,
							   get_user_id(username), 0);
	unsigned int sz_int = sizeof(int);
	lg_add_node(post->events, &current_post_id, sz_int);
	current_post_id++;
...
int find_common_repost(unsigned int post_id, unsigned int first_repost,
					   unsigned int second_repost) {
	post_t *root_post = get_post_by_id(post_id);

	unsigned int fst_rep = first_repost;
	unsigned int sec_rep = second_repost;
	unsigned int ancestor;
	ancestor = lg_lowest_common_ancestor(root_post->events,
										 get_repost_index(root_post, fst_rep),
										 get_repost_index(root_post, sec_rep));
...
unsigned int
add_repost(char *name, unsigned int post_id, unsigned int repost_id) {
	...
	if (reposted_post) {
		...
		if (repost_id == 0) {
			lg_add_edge(reposted_post->events, 0,
						get_number_reposts(reposted_post) - 1);
		} else {
			unsigned int repost_index = get_repost_index(reposted_post,
														 repost_id);
			lg_add_edge(reposted_post->events, repost_index,
						get_number_reposts(reposted_post) - 1);
		}
...
update_post_likes(char *name, unsigned int post_id, unsigned int repost_id)
{
	...
	post_t *repost = get_repost_by_index(post,
											get_repost_index(post, rep_id));
...
{
	post_t *post = get_post_by_id(post_id);
	unsigned int maximum_likes = dll_get_size(post->likes);
	unsigned int maximum_likes_id = post_id;
...
void delete_reposts_with_parent_repost(post_t *og_post,
									   unsigned int repost_index) {
	unsigned int current_node = repost_index;
	queue_t *queue = init_queue(sizeof(int));
	push_queue(queue, &current_node);
	unsigned int removed_nodes[get_number_reposts(og_post)];
	...
	while (!is_empty_queue(queue)) {
		...
		free(rm_node->data);
		free(rm_node);
		linked_list_t *list = lg_get_neighbours(og_post->events, current_node);
		ll_node_t *node = list->head;
		while (node) {
			ll_node_t *next_node = node->next;
			unsigned int next = *((int *)node->data);
			push_queue(queue, &next);
			lg_remove_edge(og_post->events, current_node, next);
			node = next_node;
		}

		post_t *rm_repost = (*((post_t **)og_post->events->data[current_node]));
		free(rm_repost->title);
		dll_free(&rm_repost->likes);
		free(rm_repost);
		ll_free(&list);
		free(og_post->events->data[current_node]);
		og_post->events->data[current_node] = NULL;
		removed_nodes[removed_counter++] = current_node;
	}

	while (removed_counter) {
		unsigned int removed_node = removed_nodes[removed_counter - 1];
		for (unsigned int i = removed_node;
			 i < get_number_reposts(og_post) - 1; i++) {
			og_post->events->data[i] = og_post->events->data[i + 1];
			og_post->events->neighbors[i] = og_post->events->neighbors[i + 1];
		}
		og_post->events->nodes--;
		unsigned int size = sizeof(void *);
		og_post->events->data = realloc(og_post->events->data,
										size * og_post->events->nodes);
		for (unsigned int i = 0; i < get_number_reposts(og_post) - 1; i++) {
			if (og_post->events->data[i]) {
				linked_list_t *list = lg_get_neighbours(og_post->events,
														i);
				ll_node_t *node = list->head;
				while (node) {
